...
void ctr_setPDEeval(void (*fpointer)(const double&, const double&, double&, 
                                 double&, double&, double&, double&, 
                                 double&, double&, double&)) {
    ctr_PDEeval = fpointer;
}

void ctr_setBCeval(void (*fpointer)(const double&, const double&, double&,
                                double&, double&, double&, double&)) {
    ctr_BCeval = fpointer;
}

void ctr_setICeval(void (*fpointer)(const double&, const double&, double&, double&)) {
    ctr_ICeval = fpointer;
}
...
int ctrdiff2D(int N, int M, 
  ...
  for(int i=1; i<=N; i++) { //actual i-index on grid
    for(int j=1; j<=M; j++) { //actual j-index on grid
      ...
      tn = t ;
      ctr_PDEeval(x(i),y(j),tn,P,Q,p,q,r,s,eta) ;
      ctr_BCeval(x(i),y(j),tn,gLeft,gRight,gBottom,gTop) ;
  ...
  for(int i=0; i<=N+1; i++){ //actual i-index on grid
    tnn = t + dt ;
    ctr_BCeval(x(i),y(M+1),tnn,gLeft,gRight,gBottom,gTop) ;
    unew(i,M+1) = gTop ; 
    ctr_BCeval(x(i),y(0),tnn,gLeft,gRight,gBottom,gTop) ;
    unew(i,0) = gBottom ; 
  }
  for(int j=0; j<=M+1; j++){ //actual j-index on grid
    tnn = t + dt ;
    ctr_BCeval(x(N+1),y(j),tnn,gLeft,gRight,gBottom,gTop) ;
    unew(N+1,j) = gRight ; 
    ctr_BCeval(x(0),y(j),tnn,gLeft,gRight,gBottom,gTop) ;
    unew(0,j) = gLeft ; 
  }
